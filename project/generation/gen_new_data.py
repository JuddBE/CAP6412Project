...
def generate(original_data, new_data, dataset_folder):
    ...
    for index, row in new_data.iterrows():
        ...
        start = sum(int(t) * x for t, x in zip(row['start_time'].split(":")[::-1], [1, 60, 3600][:len(row['start_time'].split(":"))]))
        stop = sum(int(t) * x for t, x in zip(row['end_time'].split(":")[::-1], [1, 60, 3600][:len(row['end_time'].split(":"))]))
