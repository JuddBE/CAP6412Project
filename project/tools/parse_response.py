...
class ResponseParser:
    ...
    @staticmethod
    def add_response_to_dict(prompt_idx, dict):
        parser = ResponseParser(None, prompt_idx)
        result = parser.extract_answers(dict["response"])
        for i, prompt in enumerate(parser.valid_columns):
            dict[prompt] = result[i]
        return dict
    ...
    def extract_answers(self, model_output, is_recursive=False):
        ...
        for i, options in enumerate(self.prompt_options):
            if options is None:
                ...
                modified_options[i] = [
                    option.strip().replace(" ", "-") for option in options
                ]
        ...
        if not solutions:
            ...
            for cat_idx in range(num_cats):
                ...
                max_token_count = max(
                    len(candidate.split()) for candidate in candidate_set
                )
                candidates_with_max = [
                    c for c in candidate_set if len(c.split()) == max_token_count
                ]
                ...
                if modified_options[j] is not None:
                    for k in range(len(modified_options[j])):
                        if (
                            ...
                            == modified_options[j][k].strip().lower()
                            and result[i] != self.prompt_options[j][k].strip().lower()
                        ):
...
def main():
    ...
    if id == 0:
        csv_modifier = ResponseParser(file_path, prompt_idx, output_path)
        csv_modifier.run()
    else:
        for root, dirs, files in os.walk(file_path):
            for file in files:
                if file.lower().endswith(".csv"):
                    ...
                    csv_modifier = ResponseParser(
                        csv_full_path, prompt_idx, output_path
                    )
                    csv_modifier.run()
